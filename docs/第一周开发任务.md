...
4. 配置：
   ...
   - Interface: SwiftUI
   - Language: Swift
   - 取消勾选 Core Data、Tests（后续再加）

5. 保存位置：`~/tools/Sidekick/`
...
struct DataView: View {
    ...
    var body: some View {
        VStack {
            Text("已加载 \(dataFrame.rowCount) 行数据")
                .font(.headline)
                .padding()
...
1. **小步快跑**：先实现最简单的版本，再逐步优化
2. **频繁测试**：每完成一个功能就测试
3. **及时提交**：每天提交代码到 Git
4. **记录问题**：遇到问题记录下来，方便后续解决
