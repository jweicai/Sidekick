...
**原因：**
- DuckDB 不提供原生 ORC 支持
- 需要外部依赖（如 PyArrow），违背"开箱即用"原则
- 业界主流工具（如 DBeaver）也不支持本地 ORC 文件

**替代方案：**

如果你有 ORC 文件需要查看，可以先转换为 Parquet：
...
# 通过 PyArrow 转换（需要先安装 pyarrow）
duckdb -c "INSTALL arrow; LOAD arrow; 
COPY (SELECT * FROM read_orc('file.orc')) 
...
spark.read.orc("file.orc")
  .write.parquet("file.parquet")
```

转换后的 Parquet 文件可以直接在 Sidekick 中使用。
...
**A:** 主要原因：
1. DuckDB 不提供原生 ORC 支持（只支持 Parquet）
2. 需要外部依赖（Python + PyArrow），违背"开箱即用"原则
3. 业界主流工具（如 DBeaver）也不支持本地 ORC 文件
4. Parquet 已经覆盖了 90% 的列式存储使用场景

如果你有 ORC 文件，可以使用 Spark、DuckDB CLI 或 Python 转换为 Parquet 后使用。
