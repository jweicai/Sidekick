...
TableQuery 采用插件化架构，支持动态扩展新的文件格式支持。

### 核心组件

1. **FileLoaderProtocol**：文件加载器协议
2. **FileLoaderManager**：插件管理器（单例）
3. **具体实现**：CSVLoader、JSONLoader 等
...
└──────────────┬──────────────────────────┘
       ...
       ▼               ▼           ▼
┌──────────┐    ┌──────────┐  ┌──────────┐
│CSVLoader │    │JSONLoader│  │ExcelLoader│
│  (内置)  │    │  (内置)  │  │  (插件)  │
└──────────┘    └──────────┘  └──────────┘
```
...
### 步骤1：实现 FileLoaderProtocol

创建一个新的类，实现 `FileLoaderProtocol` 协议：
...
```swift
func load(from url: URL) throws -> DataFrame {
    print("📂 [\(name)] Loading file: \(url.lastPathComponent)")
    
    let startTime = Date()
    let dataFrame = try parseFile(url)
    let duration = Date().timeIntervalSince(startTime)
    
    print("✅ [\(name)] Loaded \(dataFrame.rowCount) rows in \(duration)s")
...
1. **ExcelLoader**：支持 .xlsx, .xls
2. **ParquetLoader**：支持 Apache Parquet
3. **AvroLoader**：支持 Apache Avro
4. **XMLLoader**：支持 XML 文件
5. **SQLiteLoader**：支持 SQLite 数据库文件
...
TableQuery 的插件架构让你可以轻松扩展新的文件格式支持：

1. ✅ 实现 `FileLoaderProtocol`
2. ✅ 注册到 `FileLoaderManager`
3. ✅ 测试和发布
